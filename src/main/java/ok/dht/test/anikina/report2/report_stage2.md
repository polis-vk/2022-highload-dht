...
33% времени занимают селекты и 25% занимает one.nio чтение запроса.

### ALLOC
handleRequest занял 33% аллокаций, из них 13% происходит при записи в dao, 20% при парсинге request.
10% аллокаций использует one.nio для записи response и 2% происходят при flush на диск.
43% уходит нра селектор треды, из них 40% - чтение запроса one.nio.
...
18% уходит на чтение запроса one.nio.

### ALLOC
45% аллокаций занимает чтение из dao, 47% занимают селектор треды, из них 33% уходит на чтение запроса one.nio.

### LOCK
Аналогично с put запросами, 85% происходит при синхронизации потоков в очереди задач. 
Также 3% при синхронизации в dao и 11% приходится на селекторы.  
...
На чтение latency уменьшилась в 1.5 раза, но при этом и rate мы увеличили. Отсюда можно сделать вывод, 
...
32% занимают селекты, 26% времени занимает one.nio на чтение запроса, что не отличается от abortPolicy.
...
Разницы в производительности между abortPolicy и discardOldestPolicy особо нет, но во второй меньше времени уходит 
