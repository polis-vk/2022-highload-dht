...
class Storage implements Closeable {
    ...
    private long entryIndex(MemorySegment sstable, MemorySegment key) {
        ...
        while (left <= right) {
            ...
            long keyPos = MemoryAccess.getLongAtOffset(sstable, StorageUtils.INDEX_HEADER_SIZE
                    + mid * StorageUtils.INDEX_RECORD_SIZE);
    ...
    private Entry<MemorySegment> entryAt(MemorySegment sstable, long keyIndex) {
        try {
            long offset = MemoryAccess.getLongAtOffset(sstable, StorageUtils.INDEX_HEADER_SIZE
                    + keyIndex * StorageUtils.INDEX_RECORD_SIZE);
