...
- При бóльших объемах данных стабильная нагрузка понижается, сервер способен обработать только 10000 запросов/сек.
  За единицу времени обрабатывается меньше данных (по сравнению с 30000 запросов/сек и малым размером значений)
  \- 0.6Мб вместо 2.56Мб, т.е. понижение пропускной способности сервера не связано с тем, что данные долго ходят по
  сети или читаются из сокетов.
...
Большая часть аллокаций происходит тоже в `Storage.entryIndex`, когда данные переносятся с диска в оперативную память.
