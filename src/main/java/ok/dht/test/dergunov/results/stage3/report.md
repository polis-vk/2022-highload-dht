...
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|                                                                                                                                                                                                                                                                                                                                                                    |
| * Обработка put (метод handleRequest) запроса занимает 67.03 % времени процессора, upsert в базу данных при этом занимает 8.05 % времени, а запись в сокет (one.nio.NativeSocket.write) 36.96 % где и происходит большинство сисколов, так же весьма большое количество времени занимает логирование 16.88 %                                                       |
| * one.nio.SelectorThread 14.54 % времени процессора из которых 5.44 % времени на получение сообщения из сокета (recv), а 3 % времени на постановку задачи в очередь в LinkingBlockingQueue, при чем на метод park, блокирующий поток 1.12% метод unpark выполняющий пробуждение потока 0.77 %, а на сам же метод выполняющий постановку в очередь (enqueue) 0.08 % |
| * Работа с получением задачи для  ThreadPoolExecutor сводилась к работе с  LinkingBlockingQueue 6.43 %, большая часть из которых на метод метод park (3.77 %) выполняющий блокировку потока, и метод unpark (1.20 %) выполняющий пробуждение потока                                                                                                                |
| * one.nio.net.NativeSelector.epollWait занимает 7.95 % времени, внутри которого вызывается системный вызов epoll_wait который ожидает события epoll, он мониторит возможность  чтение записи  предоставленных файловых дескрипторов, метод используется для выбора сессии                                                                                          |
| * Сборщик мусора занимал 1.45 % времени                                                                                                                                                                                                                                                                                                                            |
...
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| * 70.68 % алолокаций идет на чтение запроса (one.nio.http.HttpSession.processRead), где происходит распарсинг запроса из wrk                                                                                                                                                                                                                                                                                                                      |
| * Получение пути запроса (getPath) 3.14 %                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| * При работе с SelectorThread 2.96 % аллокаций                                                                                                                                                                                                                                                                                                                                                                                                    |
...
|----------------------------------------------------------------------|
|                                                                      |
| * 37.3 % локов идет на помещение задач в ArrayBlockingQueue (offer)  |
| * 34.03 % локов на работу с сессиями (one.nio.http.Session)          |
| * 26.5 % локов идет на получение задачи из ArrayBlockingQueue (take) |
...
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|                                                                                                                                                                                                                                                                                                                                                                     |
| * Обработка get (метод handleRequest) запроса занимает 67.35 % времени процессора, get в базу данных при этом занимает 16.91 % времени, а запись в сокет (one.nio.NativeSocket.write) 47.06 % где и происходит большинство сисколов                                                                                                                                 |
| * one.nio.SelectorThread 16.84 % времени процессора из которых 7.14 % времени на получение сообщения из сокета (recv), а 1.92 % времени на постановку задачи в очередь в ArrayBlockingQueue, при чем на метод park, блокирующий поток 0.97% метод unpark выполняющий пробуждение потока 0.65 %, а на сам же метод выполняющий постановку в очередь (enqueue) 0.26 % |
| * Работа с получением задачи для ThreadPoolExecutor сводилась к работе с  ArrayBlockingQueue 4.33 %, большая часть из которых на метод метод park (2.57 %) выполняющий блокировку потока, и метод unpark (0.88 %) выполняющий пробуждение потока                                                                                                                    |
| * one.nio.net.NativeSelector.epollWait занимает 8.37 % времени, внутри которого вызывается системный вызов epoll_wait который ожидает события epoll который мониторит возможность  чтение записи  предоставленных файловых дескрипторов, метод используется для выбора сессии                                                                                       |
| * Сборщик мусора занимал 0.58 % времени                                                                                                                                                                                                                                                                                                                             |
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|                                                                                                                                                                                                                                                                                                                                               |
| * 69.91 % алолокаций идет на чтение запроса (one.nio.http.HttpSession.processRead), где происходит распарсинг запроса из wrk                                                                                                                                                                                                                  |
| * Получение пути запроса (getPath) 1.6 %                                                                                                                                                                                                                                                                                                      |
...
| * При работе с SelectorThread 3.36 % аллокаций                                                                                                                                                                                                                                                                                                |
...
|-----------------------------------------------------------------------|
|                                                                       |                                      
| * 39.54  % локов идет на помещение задач в ArrayBlockingQueue (offer) |
| * 30.28 % локов на работу с сессиями (one.nio.http.Session)           |
| * 27.67 % локов идет на получение задачи из ArrayBlockingQueue (take) |
