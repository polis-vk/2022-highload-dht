...
`wrk -d 60 -t 64 -c 64 -R 150000 -s ./put.lua -L http://localhost:8084`
...
| * Обработка put (метод handleRequest) запроса занимает 62.4 % времени процессора при этом метод sendResponse занимает 50.55 % общего времени, upsert в базу данных при этом занимает 8.81 % времени |                                 |
| * SelectorThread one.nio 18.55 % времени процессора                                                                                                                                                 |                                 |
| * Работа с ArrayBlockingQueue 5.41 % времени при этом метод park                                                                                                                                    |                                 |
...
`wrk -d 60 -t 64 -c 64 -R 150000 -s ./get.lua -L http://localhost:8084`
