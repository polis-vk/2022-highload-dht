...
CPU:
    Из результатов профилирования следует, что обработка запроса (one.nio.net.Session.process) занимает 72.8%
    процессорного времени, из которых только 1.92% - выполнение метода upsert в dao, большая часть времени уходит 
    на работу с сетью (RequestHandler1_handlePut.handleRequest - 56.18%), а также, нативный метод
    one.nio.net.NativeSocket.read - 10.44%.
    Также 0.74% занимает запись данных на диск (flush). 
    Остальное время отводится на native код, в основном: one.nio.net.NativeSelector.epollWait - 10.85%. 
...
Alloc:
    ...
    Остальное - для обработки запроса с помощью one.nio - более 60% аллокаций.
...
CPU:
    Из результатов профилирования следует, что обработка запроса (one.nio.net.Session.process) занимает 73.72%
    процессорного времени, из которых 23.1% - выполнение метода get в dao, где большая часть времени уходит
    на бинарный поиск - 16.32%, остальное на инициализацию MergeIterator.
    Работа с сетью (RequestHandler1_handlePut.handleRequest - 37.17%), а также, нативный метод
    one.nio.net.NativeSocket.read - 7.13%.
    Остальное время отводится на native код, в основном: one.nio.net.NativeSelector.epollWait - 9.05%.
