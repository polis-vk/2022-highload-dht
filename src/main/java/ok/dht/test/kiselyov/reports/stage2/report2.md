...
Запросы с спользованием 'ExecutorService' в 64 потока и 64 соединения в асинхронном режиме с использованием стека
-
...
числа потоков до 20 среднее время выполнения запроса сократилось до ~800us для GET и PUT запросов. В асинхронном режиме
...
рейта. GET запросы на 4 потоках показали результат в ~1s-6s в зависимости от рейта. При увеличении потоков до 20 время

сократилось значительно - до ~630ms-1.15s в зависимости от рейта. Можно сделать вывод, что асинхронный сервис на 

`ExecutorService` не очень хорошо себя показывает на небольшом количестве одновременно работающих потоках, но ситуация

улучшается при увеличении числа потоков. Также стоит отметить, что замена стека на очередь ухудшила среднее latency на
