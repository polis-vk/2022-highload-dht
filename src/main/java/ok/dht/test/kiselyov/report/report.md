...
> Хитмап и флеймграф в файлах put_alloc.html и put_cpu.html.
...
> Хитмап и флеймграф в файлах get_alloc.html и get_cpu.html.
...
составляет примерно 0.83s. 90% запросов выполняются за 1.12s в среднем, а 99% - за 15.04s. При скорости обработкиэ

запросов в 90000 r/s и более сервис начинает тормозить и latency в среднем составляет уже примерно 2.3s и будет

постепенно увеличиваться. 90% запросов при такой скорости выполняются за 5.31s в среднем, а 99% - уже за 31.14s.
...
составляет примерно 2.11s. 90% запросов выполняются за 5.82s в среднем, а 99% - за 6.39s. При скорости обработкиэ

запросов в 30000 r/s и более сервис начинает тормозить и latency в среднем составляет уже примерно 29.91s и будет

постепенно увеличиваться. 90% запросов при такой скорости выполняются за 65.60s в среднем, а 99% - уже за 131.68s.

Процессорное время делится следующим образом: примерно 55% времени занимает работа по поиску записей в БД, еще около 25% -

обмен информации по протоколу tcp/ip. Также 20% - работа библиотеки one-nio.
