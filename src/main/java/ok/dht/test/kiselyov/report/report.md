...
Оставшееся время (приблизительно 60%) приходится на JIT-компиляцию и flush данных (особенно на поздних этапах, когда

записи становятся очень длинными и приходится часто обращаться к памяти).

Память распределяется следующим образом: приблизительно 50% памяти выделяется на генерацию объекта ArrayQueueItem из

класса one.nio.net.Session. Данный объект представляет собой набор данных, которые передаются клиентом и оборачиваются

в определенную структуру. Оставшаяся память (приблизительно 50%) выделяется под парсинг строк (для
...
удаления). Наибольшую проблему представляют селекторы, которые блокируются при большом количестве попыток соединения

от клиентов. Для улучшения данной операции необходимо добавить очередь из запросов и воркеров, которые будут эти

очереди обрабатывать.
...
Рост времени на высоких процентилях вызван JIT-компиляцией на начальных этапах, работой сборщика мусора. Наибольшую

проблему так же представляют селекторы, которые не позволяют работать с большим количеством клиентов. Необходимо

добавить очередь из запросов и воркеров, которые будут обрабатывать очереди.