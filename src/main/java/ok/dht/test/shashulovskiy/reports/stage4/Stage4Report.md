...
Тестировать я буду с 3 шардами в кластере и c дефольным кворумом (ack = 2/from = 3).
...
Обратим внимаение, что по сравнению с шардированной реализацией из прошлого дз, мы видим рпс ниже порядка в 3 раза.
...
много работы на формирование запроса и обработку ответа, ожидаение соединения, ожидание ответа и т.д. Очевидным решением тут было 
...
# Выводы
1. Репликации очевидно не дала выигрыша в производительности, однако сильно подняла нашу отказоустойчивость, так как мы 
теперь можем перешить отказ какого-то количества узлов. Замедление работы же примерно пропорционально кол-во реплик на которые
мы делаем запросы
2. Основной оверхед все еще несет использование HTTP. Для внутреннего апи хорошо было бы использовать байтовый RPC, поверх
UPD с минимумом накладных расходов.
