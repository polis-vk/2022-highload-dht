...
Сравним с синхронной реализацией:
...
Кол-во аллокаций сильно выросло. Если раньше аллокации селектора занимали 34 процента от общего числа, то теперь это
всего-лишь 2 процента. Основным спонсором аллокаций стали CompletableFuture и доп расходы на асинхронную обработку
...
Здесь профиль такой же как при PUT'е, однако видно больше работы на самой операции чтения из базы, так как в отличии от
...
5. Все еще актуалена оптимизация по переходу с HTTP на RPC протокол с меньшим оверхедом