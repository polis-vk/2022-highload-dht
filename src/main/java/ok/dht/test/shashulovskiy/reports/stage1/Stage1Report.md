...
Попробуем сначала такой тест: данных которые мы ищем почти всегда нет. 10000 RPS мы как и до этого
спокойно держим. 
...
2. Распределение ключей по которым идет поиск не оказывает большого влияния на выдерживаемую
нагрузку. Оптимальной конечно является ситуация когда мы лучше попадаем в кеш и уже загруженные данные.
3. Ускорить GET запросы потенциально можно было бы добавлением небольшого кеша. Это бы сильно улучшило
...
Во-первых, очень много аллокаций уходит на "профилактические" работы внутри самой LevelDB, очевидно помогло
...
логично, ведь это GET. В самом поиске аллокации тратятся на то чтобы прочитать данные и сравнить из с тем
что мы ищем, и непосредственно отпавить ответ. 
