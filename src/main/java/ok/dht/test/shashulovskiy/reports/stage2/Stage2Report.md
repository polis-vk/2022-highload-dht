...
## Реализация на Executor Service (многопоточная реализация). Дефольные настрйки
...
Будем использовать чисо потоков, равное кол-ву ядер в процессоре. В моем случае - 10.
...
Получили заметое незначительно уменьшение в кол-ве обработанных PUT запросов и значительное
...
в случае PUT запросов, и на 13% в случае GET запросов (реализация без пула vs. LIFO based pool). Однако,
по результатам профилирования, я бы списал это скорее на погрешности замеров.
...
Как видим, здесь ситуация все еще такая же. Подробно происходящие я описывал в прошлом
...
Еще стоит заметить что на хитмапе очень много белых или почити белых точек. Если рассмотреть что в них
...
1. Несмотря на то, что нагрузочное тестирование показало прирост по RPS, мне кажется большую его часть
...
4. При анализе политик поддержания очереди лучше всего себя показала LIFO очередь, но тут есть простор для 
