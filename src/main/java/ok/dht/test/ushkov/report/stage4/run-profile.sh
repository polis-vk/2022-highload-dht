...
function async-profiler-start() {
    "$ASYNC_PROFILER_DIR/profiler.sh" -f "$DIR/profile.jfr" -e "cpu,alloc,lock" start 37256
}

function async-profiler-stop() {
    "$ASYNC_PROFILER_DIR/profiler.sh" stop 37256 > /dev/null
}
...
echo "=== Start wrk2"
wrk2 -t "$t" -c "$c" -R "$R" -d "$d" -s "scripts/$SCRIPT" -L http://localhost:1337 > "$DIR/wrk2.txt"
