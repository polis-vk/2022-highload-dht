...
function async-profiler-start() {
    "$ASYNC_PROFILER_DIR/profiler.sh" -f "$DIR/profile.jfr" -e "cpu,alloc,lock" start MainProfiling
}

function async-profiler-stop() {
    "$ASYNC_PROFILER_DIR/profiler.sh" stop MainProfiling > /dev/null
}
...
echo "=== Start wrk2"
wrk2 -t "$t" -c "$c" -R "$R" -d "$d" -s "scripts/$SCRIPT" -L \
"http://localhost:${PROFILING_PORT}" > "$DIR/wrk2.txt"
