...
class Storage implements Closeable {
    ...
    static void save(
        ...
        final Path sstablePath = config.basePath().resolve(FILE_NAME + nextSSTableIndex + FILE_EXT);
    ...
    public static void compact(final Config config, final Data data) throws IOException {
        final Path compactedFile = config.basePath().resolve(COMPACTED_FILE);
        save(data, compactedFile);
        finishCompact(config, compactedFile);
    }

    private static void finishCompact(final Config config, final Path compactedFile) throws IOException {
        for (int i = 0; ; i++) {
    ...
    private long entryIndex(final MemorySegment sstable, final MemorySegment key) {
        ...
        while (left <= right) {
            ...
            final long keyPos = MemoryAccess.getLongAtOffset(sstable, INDEX_HEADER_SIZE + mid * INDEX_RECORD_SIZE);
    ...
    private Entry<MemorySegment> entryAt(final MemorySegment sstable, final long keyIndex) {
        try {
            final long offset = MemoryAccess.getLongAtOffset(sstable, INDEX_HEADER_SIZE + keyIndex * INDEX_RECORD_SIZE);
