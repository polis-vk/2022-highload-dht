...
public final class StorageHelper {
    ...
    static void save(
        ...
        try (@SuppressWarnings("LocalCanBeFinal") ResourceScope writeScope = ResourceScope.newConfinedScope()) {
            ...
            for (final Entry<MemorySegment> entry : new IterableIteratorWrapper<>(entries.getIterator())) {
                ...
                offset += writeRecord(nextSSTable, offset, entry.getKey());

                MemoryAccess.setLongAtOffset(nextSSTable, offset, entry.getTimestamp());
                offset += Long.BYTES;

                offset += writeRecord(nextSSTable, offset, entry.getValue());
    ...
    private static long getSize(final Entry<MemorySegment> entry) {
        if (entry.getValue() == null) {
            return Long.BYTES + entry.getKey().byteSize() + Long.BYTES + Long.BYTES;
        } else {
            return Long.BYTES + entry.getKey().byteSize() + Long.BYTES + Long.BYTES + entry.getValue().byteSize();
        }
    }
