...
import ok.dht.test.skroba.server.response.ChunkedQueueItem;
...
public class ConcurrentHttpServer extends HttpServer {
    ...
    private static final Set<Integer> SUPPORTED_METHODS = Set.of(Request.METHOD_GET, Request.METHOD_PUT,
    ...
    public ConcurrentHttpServer(final EntityDao dao, final Manager manager, final HttpServerConfig config,
        ...
        handlers = Arrays.stream(AcceptedPaths.values())
                .collect(Collectors.toMap(AcceptedPaths::getPath, it -> it.getHandler(manager, dao)));
    }
    ...
    public synchronized void start() {
        ...
        requestWorkers = new ThreadPoolExecutor(CORE_POOL_SIZE, MAXIMUM_POOL_SIZE, 0L, TimeUnit.MILLISECONDS,
    ...
    public void handleRequest(final Request request, final HttpSession session) throws IOException {
        ...
        try {
            requestWorkers.submit(() -> processRequest(request, session));
    ...
    private void processRequest(final Request request, final HttpSession session) {
        final RequestHandler handler = handlers.get(request.getPath());
        
        try {
            handler.handle(request, session, forJoin);
