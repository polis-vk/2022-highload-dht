...
реализации шардирования в несколько раз увеличили количество пересылаемых requests и responses. Также в 
данном случае хотелось бы найти альтернативу java Http client, потому что текущая реализация с Сompletable future вызывает 
get и паркуется, что кажется не самым эффективным. Можно попытаться реализовать асинхронный клиент, что в данном 
случае пытается сделать стандартная реализация вызывая под капотом sendAsync, однако вызов get все равно создает активное ожидание.
...
чем вариант с консистентным хэшированием, но так как кол-во кластеров небольшое, то оно работает лучше.
...
Заметим, что выделение памяти сильно усложнилось аллокация памяти для селекторов уменьшилась в процентном соотношении с
24% до 14%.
