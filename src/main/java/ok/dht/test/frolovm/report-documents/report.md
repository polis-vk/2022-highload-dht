...
При rate=20000 сервер захлебнулся и среднее Latency 4.45s. \
...
На запрос в бд тратится 5% времени. Также запускается flush(), процент которого вырос от 10k запросов к 20k запросам
с 0.88% до 2% из-за увеличения кол-ва данных.

Основная часть запросов к ServiceImpl приходится на ConcurrentSkipListMap put(~3% от общего CPU), который в свою очередь
вызывает MemorySegmentComparator.compare, так как ConcurrentSkipListMap поддерживает такие интерфейсы как SortedMap, NavigableMap, and ConcurrentNavigableMap.
Поэтому при проектировании базы данных стоит уделять внимание эффективности реализации функции compare для ее содержимого.
...
При rate=7000 сервер захлебнулся и среднее Latency 5.73s . \
...
Основное время тратится на getOffset из jdk.incubator.foreign.MemoryAccess и 
...
Также как при put запросах значительная часть CPU тратится на сравнение элементов, поэтому важна эффективность реализации функции compare для содержимого базы.
