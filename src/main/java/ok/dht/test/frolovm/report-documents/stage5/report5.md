...
Http Сlient и для Completeble future, как было сказано в видео на данный момент в Java - данное решение наиболее быстрое.
...
Эти данные позволяют сделать выводы о целесообразности использования асинхронного решения. Также не стоит забывать, что 
в реальном продакшене затраты на сетевую передечу данных будут выше, чем в данные момент на локальной машине, поэтому 
перфоманс данного решения только вырастет.
