...
Результаты для lock не отличаются от предыдущего этапа 99% lock занимает HTTP client.
...
Стало больше аллокаций, мы стали хранить еще больше данных, однако затраты соответствуют ожиданиям. При этом мы храним 
большое количество неактуальных данных, хорошей фичей было бы добавить рассылку актуальных значений при GET запросе нодам, 
на которых значение на данный момент не явялется актуальным.
