...
версии было 12%. В основном связано с перераспределением процентов.
...
Много аллокаций приходится Completeble future complte на handle response в Http Client, так доля в процентах выросла с 
...
при GET запросе нодам, на которых значение не является актуальным.
